<?php
  ...
  $query = "SELECT count(sl) FROM `inout` WHERE date='$date' and gender='Male' and status='IN' and loc='$loc'";
  $result = mysqli_query($conn, $query) or die("Invalid query: " . mysqli_error($conn));
  $male = mysqli_fetch_row($result);
  $query = "SELECT count(sl) FROM `inout` WHERE date='$date' and gender='Female' and status='IN' and loc='$loc'";
  $result = mysqli_query($conn, $query) or die("Invalid query: " . mysqli_error($conn));
